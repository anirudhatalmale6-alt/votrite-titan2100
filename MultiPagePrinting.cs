...
namespace VotRite
{
    public class MultiPagePrinting
    {
        ...
        private Bitmap panel_;

        public MultiPagePrinting(Bitmap pnl)
        {
            panel_ = pnl;
        ...
        {
            list = new List<Image>();
            Graphics g = Graphics.FromImage(panel_);
            Brush redBrush = new SolidBrush(Color.Red);
            Pen pen = new Pen(redBrush, 3);
            decimal xdivider = panel_.Width / 595m;
            int xdiv = Convert.ToInt32(Math.Ceiling(xdivider));
            decimal ydivider = panel_.Height / 841m;
            int ydiv = Convert.ToInt32(Math.Ceiling(ydivider));
            /*int xdiv = panel_.Width / 595; //This is the xsize in pt (A4)
            int ydiv = panel_.Height / 841; //This is the ysize in pt (A4)
            ...
            for (int i = 0; i < xdiv; i++)
            {
                for (int y = 0; y < ydiv; y++)
                {
                    ...
                    try
                    {
                        r = new Rectangle(i * Convert.ToInt32(panel_.Width / xdiv),
                                                    y * Convert.ToInt32(panel_.Height / ydiv),
                                                    panel_.Width / xdiv,
                                                    panel_.Height / ydiv);
                    }
                    catch (Exception)
                    {
                        r = new Rectangle(i * Convert.ToInt32(panel_.Width / xdiv),
                          y * Convert.ToInt32(panel_.Height),
                          panel_.Width / xdiv,
                          panel_.Height);
                    }
                    ...
                    list.Add(cropImageN(panel_, r));
                }
            }

            g.Dispose();
           
           // panel_ = (Bitmap) list[0];
        ...
        private static Image cropImageN(Image img, Rectangle cropArea)
        {
            Bitmap bmpImage = new Bitmap(img);
            Bitmap bmpCrop = bmpImage.Clone(cropArea, System.Drawing.Imaging.PixelFormat.DontCare);
            return (Image)(bmpCrop);
