...
namespace VotRite.Models
{
    class SlatesModel : ScreenModel
    {
	    ...
	    public SlatesModel()
		{
            ...
            ContestDefinition.SetContestFields(ref contest, ref currentRaceInstance, ref currentPropositionInstance, session.CurrentLocale, null, true);
            ContestDefinition.FillContestData(ref contest, currentPropositionInstance, session.CurrentLocale);
            DefinitionParser.Instance.FillSlatesContent(ballot, session.CurrentLocale, null);
		...
		public SlatesView GetView()
		{
			SlatesView v = null;
			
			foreach (IObserver obs in observers)
			{
				if (obs.GetType().ToString() == "VotRite.Views.SlatesView")
				{
					v = (SlatesView)obs;
					break;
				}
