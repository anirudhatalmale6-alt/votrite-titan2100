...
{
    class CountiesController : ScreenController
    {
        private new CountiesModel model;
        public ScreenObject ScrObject { get; private set; }

        public CountiesController(CountiesModel m)
            : base(m)
        {
            model = m;
        }
                    ...
                    if (scrObj.Visible)
                    {
                        switch (scrObj.Name)
                        {
                            case "btn_next":
                                ...
                                if (model.Ballot.ContestsList.Count < 1)
                                {
                                    return;
                                }
                                ...
                                {
                                    County c = County.GetCounty(model.Ballot.CountyId);

                                    if (c != null)
                                    {
                                        model.Ballot.Address = c.Name + " - " + model.Ballot.Address;
                                    }
                            ...
                            case "btn_review":
                                ...
                                break;
                            //////case "btn_audio":
                            //////    AppManager.Instance.SetScreen("speechRate", "Parties");
                            //////    break;
                            //////case "btn_textsize":
                            //////    AppManager.Instance.SetScreen("textSize", "Parties");
                            //////    break;
                            case "btn_setting_tools":
                                AppManager.Instance.SetScreen("settingTools", "Counties");
                                break;
                            case "btn_callhelp":
                                ...
                                //F_CallHelp oHelp = new F_CallHelp();
                                //oHelp.ShowDialog();
                                break;
                        }
                    }
